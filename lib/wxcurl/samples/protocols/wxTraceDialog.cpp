...
// wxTraceDialog.cpp: implementation of the wxTraceDialog class.
//
//////////////////////////////////////////////////////////////////////
...
IMPLEMENT_CLASS(wxTraceDialog, wxDialog)
BEGIN_EVENT_TABLE(wxTraceDialog, wxDialog)
	EVT_BUTTON(XRCID("file_menu_trace"), wxTraceDialog::OnTrace)
END_EVENT_TABLE()
...
{
	wxXmlResource::Get()->LoadDialog(this, pParent, wxS("trace_dialog"));
...
void wxTraceDialog::OnTrace(wxCommandEvent& WXUNUSED(event))
{
	...
	if(m_pHostCtrl && m_pUserCtrl && m_pPassCtrl)
	{
		...
		if(http.Trace())
		{
			if(m_pTextCtrl)
			{
				m_pTextCtrl->SetValue(http.GetResponseBody());
			}
		}
	}
}
